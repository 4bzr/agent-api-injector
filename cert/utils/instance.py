from cert.utils.base import Window, Process, RBXString
from cert.utils.classdescriptor import ClassDescriptor
from cert.utils.utils import Offsets, GetRenderViewFromLog
...
class Instance:
    ...
    def Value(self):
        ...
        if self.ClassName == "BoolValue":
            ...
            return Instance(Process.read_longlong(self.Address + Offsets.ValueBase))
    ...
    def GetDescendants(self):
        return self.GetChildren(self.Address)
...
def FetchRenderView(process_id: int) -> int:
    ...
    try:
        RenderViewAddy = GetRenderViewFromLog()

        if not RenderViewAddy:
            pass

    except Exception as bruhmoment:
        error("Failed to fetch RenderView, info:", bruhmoment)
