from cert.utils.instance import Instance, Process
from typing import Callable

from cert.utils.logger import debug, info, error, bridge, send_message
...
class BridgeChannel:
    ...
    def Initialize(self, channel_container: Instance):
        ...
        bridge(self.States, self.Peer0, self.Peer1, self.InstanceRefs)
    ...
    def GetBufferData(self, container_type: int) -> str:
        ...
        if len(result) > 0:
            ...
            if buffer_size:
                ...
                error(f"Failed to get buffer size of '{result}'")
                return ""
        ...
        for buffer_pos in range(1, len(new_data) + 1, data_maxlen):
            ...
            if buffer_idx in buffers_cache:
                ...
                if buffer_obj.Address > 1000:
                    ...
                    error(f"Failed to send data '{new_data[0:128]}...' to roblox")
                    return False
...
class Bridge:
    ...
    def Start(self, new_pid: int, MainContainer: Instance):
        ...
        bridge(channels, self.ModuleHolder)
        ...
        for channel_idx in range(8): # 8 channels (hardcoded becus crappy children counting)
            ...
            if channel_container.Address < 1000:
                error(f"Cannot fetch BridgeChannel '{channel_idx}'")
                continue

            channel_obj = BridgeChannel()
            channel_obj.Initialize(channel_container)
            # bridge(f"Bridge '{channel_idx}' was opened!")
    ...
    def Send(self, action, args: list[any]=[]):
        ...
        try:
            ...
            error(f"error sending action ({action}, {self.Sessions[action]}) with args {args}")
    ...
    while True:
        ...
        for channel in self.Channels:
            try:
                ...
                error(f"Failed to decode data of channel {channel}")
                info(bruhmoment)
                channel.SetChannelStates(False, False, False, SENDER_TYPE["E2R"])
